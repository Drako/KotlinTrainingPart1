...
interface PersonRepository : PagingAndSortingRepository<Person, Int> {
  @Modifying
  @Query("UPDATE Person p SET p.firstName = ?2 WHERE p.id = ?1")
  fun updateFirstName(id: Int, firstName: String)

  @Modifying
  @Query("UPDATE Person p SET p.lastName = ?2 WHERE p.id = ?1")
  fun updateLastName(id: Int, lastName: String)

  @Modifying
  @Query("UPDATE Person p SET p.firstName = ?2, p.lastName = ?3 WHERE p.id = ?1")
